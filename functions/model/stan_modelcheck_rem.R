...
stan_diagnostic <- function(fit, 
                            results_folder,
                            params = c('gamma_i', 'ndd_betaij', 'weight', 'disp_dev'),
                            ...) {
  ...
  # Diagnostic plots -----------------------------------------------------------------------
  # We ignore the sign-switching response and effect params and only evaluate those of interest
  png(paste0(results_folder, '/diag_rhat.png'), width=800, height=800)
  x <- stan_rhat(fit, pars = params)  # distribution of Rhat
  print(x)
  dev.off()
  png(paste0(results_folder, '/diag_effSS_totalSS.png'), width=800, height=800)
  x <- stan_ess(fit, pars = params)   # ratio of effective sample size to total sample size
  print(x)
  dev.off()
  png(paste0(results_folder, '/diag_MCse_postsd.png'), width=800, height=800)
  x <- stan_mcse(fit, pars = params)  # ratio of Monte Carlo standard error to posterior standard deviation
  print(x)
  dev.off() 
...
### This function checks traceplots and posterior intervals ###
stan_model_check <- function(fit,
                             results_folder,
                             params = c('disp_dev', 'gamma_i', 'effect', 'response'),
                             ...) {
...
stan_post_pred_check <- function(mu,
                                 disp_dev,
                                 var_name = 'mu',
                                 results_folder,
                                 stan.data,
