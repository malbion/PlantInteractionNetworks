...
efc <- function(imat, com, intras, ...) {
  fecund <- read.csv(paste0('clean_data/fecundities', com, '.csv'), stringsAsFactors = F)
...
#-------------------------------------------------------------------------------------------
# Given a species x neighbour x samples array of scaled interaction strengths (alpha) 
...
calc.sp.ntwk.metrics <- function(
  ...
  # COMPETITIVE RANK
  #-------------------------------------------------------------------------------------------
  # aka how many species do you outcompete in a binary competitive outcomes matrix
